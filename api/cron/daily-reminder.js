...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Rodadas abertas de todos os tenants, agrupadas
    const roundsSnap = await getDocs(collection(db, 'rounds'));
    const openByTenant = {};
    roundsSnap.docs.forEach(d => {
      const r = { id: d.id, ...d.data() };
      if (r.status !== 'open' || !r.tenantId) return;
      (openByTenant[r.tenantId] = openByTenant[r.tenantId] || []).push(r);
    });

    for (const tenant of tenants) {
      const openRounds = openByTenant[tenant.id] || [];
      if (!openRounds.length) continue;
      ...
      for (const round of openRounds) {
        ...
        if (round.lembreteEnviadoData === todayBRT) {
          logs.push(`[${tenant.id}] Lembrete de ${roundName} já enviado hoje (${todayBRT}). Pulando.`);
          continue;
        }
        ...
        if (pendingUsers.length === 0 && !GROUP_JID) {
          logs.push(`[${tenant.id}] Todos os participantes já apostaram na ${roundName}.`);
          continue;
        }
